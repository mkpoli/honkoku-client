...
| Viewer modes: normal, 画像を保存 (clip), 注釈・翻刻を追加 (region note), AI recognition (range select, Metom or in-browser model) | ○ | △ normal, clip, Metom recognition | Recognition candidates support caret insertion and OCR-line segments. Region notes and in-browser recognition remain absent. |
