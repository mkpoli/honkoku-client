...
| 割書 | inline | `《割書：一行目｜二行目｜三行目》` (2–4 segments, `｜`-separated); a lone segment with no `｜` is the right half alone and does occur in transcriptions | split/interlinear writing (warigaki) |
